...
def _extract_json_archive(archive_path: Path, dest: Path) -> Path:
    """Extract the first .json file from a .tgz/.tar.gz/.zip archive to dest."""
    if dest.exists() and dest.stat().st_size > 10:
        return dest
    if archive_path.suffix == ".zip":
        with zipfile.ZipFile(archive_path) as zf:
            ...
            if not member:
                raise ValueError(f"no JSON member found in {archive_path.name}")
            log.info("  extracting: %s", member)
            dest.write_bytes(zf.read(member))
    else:
        with tarfile.open(archive_path, "r:gz") as tf:
            member = next((m for m in tf.getmembers() if m.isfile() and m.name.endswith(".json")), None)
            if not member:
                raise ValueError(f"no JSON member found in {archive_path.name}")
...
def _category_text(cat: object) -> str:
    """Normalise Wiktionary category values, which may be strings or dicts."""
    if isinstance(cat, str):
        return cat
    if isinstance(cat, dict):
        return str(cat.get("name") or cat.get("category") or cat.get("text") or "")
    return str(cat or "")
...
def capture_ja() -> None:
    ...
    # JMdict simplified JSON (scriptin's pre-parsed version).  The GitHub
    # release tag changes weekly, so try current known releases and extract the
    # JSON member from the tarball instead of gunzipping it as if it were a raw
    # .json.gz file.
    jmdict_tgz = RAW_DIR / "jmdict-eng.json.tgz"
    jmdict_path = RAW_DIR / "jmdict-eng.json"
    candidates = [
        (
            "https://github.com/scriptin/jmdict-simplified/releases/download/"
            "3.6.2%2B20260420131912/jmdict-eng-3.6.2+20260420131912.json.tgz",
            "scriptin-20260420-tgz",
        ),
        (
            "https://github.com/scriptin/jmdict-simplified/releases/download/"
            "3.6.2%2B20260413165336/jmdict-eng-3.6.2+20260413165336.json.tgz",
            "scriptin-20260413-tgz",
        ),
        (
            "https://github.com/scriptin/jmdict-simplified/releases/download/"
            "3.6.2%2B20260406125001/jmdict-eng-3.6.2+20260406125001.json.tgz",
            "scriptin-20260406-tgz",
        ),
    ]
    try:
        _dl_first(candidates, jmdict_tgz, "JMdict simplified JSON")
        _extract_json_archive(jmdict_tgz, jmdict_path)
        log.info("  parsing JMdict ...")
        with open(jmdict_path, encoding="utf-8") as f:
            jmdict = json.load(f)
    except Exception:
        # A common failure mode is a cached HTML error page or a raw tar file
        # left at jmdict-eng.json by the older _gunzip() path.  Remove cached
        # artefacts once and retry with the candidate list.
        for stale in (jmdict_tgz, jmdict_path):
            if stale.exists():
                stale.unlink()
        _dl_first(candidates, jmdict_tgz, "JMdict simplified JSON")
        _extract_json_archive(jmdict_tgz, jmdict_path)
...
def capture_zh() -> None:
    ...
    for level in range(1, 7):
        ...
        candidates = [
            (
                f"https://raw.githubusercontent.com/clem109/hsk-vocabulary/master/"
                f"hsk-vocab-json/hsk-level-{level}.json",
                "clem109-json",
            ),
            (
                f"https://raw.githubusercontent.com/kfcd/chaizi/master/data/hsk{level}.txt",
                "kfcd-hsk-txt",
            ),
            (
            ...
            if raw.startswith("[") or raw.startswith("{"):
                ...
                if isinstance(parsed, list):
                    if parsed and isinstance(parsed[0], str):
                        ...
                        # Common formats include [{Simplified, ...}], [{hanzi, ...}],
                        # and [{simplified, ...}].
                        words = [
                            e.get("Simplified") or e.get("simplified") or
                            e.get("hanzi") or e.get("word") or e.get("text") or ""
...
def capture_he() -> None:
    ...
    try:
        ...
        for v in verbs:
            cats = [_category_text(c) for c in v.pop("categories", [])]
...
def capture_grc() -> None:
    ...
    # MorphGNT — per-word morphological codes for the Greek NT.  The old
    # combined MorphGNT_SBLGNT.txt URL now 404s; the repository stores one
    # *-morphgnt.txt file per book, so download the repo zip and concatenate
    # those files locally.
    try:
        morphgnt_zip = RAW_DIR / "morphgnt_sblgnt_master.zip"
        ...
        if not morphgnt_path.exists() or morphgnt_path.stat().st_size <= 10:
            log.info("  extracting MorphGNT book files ...")
            with zipfile.ZipFile(morphgnt_zip) as zf:
                members = sorted(n for n in zf.namelist() if n.endswith("-morphgnt.txt"))
                if not members:
                    raise ValueError("no *-morphgnt.txt files found in MorphGNT archive")
                with open(morphgnt_path, "wb") as out:

...
def test_build_check_prints_concise_ok_not_report_table():
    ...
    assert result.stdout.strip() == "OK: validated 483 entries across 17 languages"
...
def test_write_out_dir_writes_files_and_prints_concise_summary(tmp_path):
    ...
    assert result.stdout.strip() == f"Wrote 17 catalogue files to {tmp_path} (483 entries)"
    assert "language | entries" not in result.stdout
...
def test_generated_json_determinism_in_temp_dir(tmp_path):
    ...
    assert first.stdout.strip() == f"Wrote 17 catalogue files to {first_dir} (483 entries)"
    assert second.stdout.strip() == f"Wrote 17 catalogue files to {second_dir} (483 entries)"

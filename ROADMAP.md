...
- **Classical lexicon depth** — **done** (2026-05-29): Latin noun/adj coverage expanded from ~3 400 → ~257 000 forms (`la_morph.json`, kaikki Wiktionary paradigm tables). Latin verb coverage added: ~615 000 verb forms across ~5 400 lemmas in `la_verb_morph.db` (SQLite, lazily opened per-connection, indexed lookup). Priority: suffix rules (infinitive/imperfect/future) → SQLite (present/perfect/subjunctive/passive/participle/etc.) → vocabulary. Latin `morphology_quality` upgraded `"low"` → `"medium"`. Greek unchanged at ~27 000 forms. Perseus/Logeion API integration deferred.

...
- **Shallow morphological plugins for Hindi, Turkish, and Finnish** — **done** (2026-05-27): all three added as suffix-rule `morphology_light` plugins. Hindi: Devanagari word regex, IAST romanisation, verb aspect/tense and noun case suffix rules, postposition tagging. Turkish: vowel-harmony-aware suffix rules for plural, case, tense, and mood (false-positive-prone short suffixes removed; confirmed tests pass). Finnish: 15-case suffix detection (inessive through comitative), vowel harmony, 58 tests in `test_hindi_turkish_finnish_plugins.py`. All three declared with `morphology_quality="low"` and appropriate nuance_capabilities. Full spaCy-model morphology for Korean and the deeper Indian/Turkic/Finnic coverage remain **planned**.
- **CEFR A2 vocabulary** — **planned**: extend `cefr_vocab.py` with `A2` tables (~800–1000 lemmas per language). A2 adds "elementary" everyday expressions — extended family, work/school topics, travel, simple descriptions. Plugin `_vocab_confidence` priority: A1 (0.90) → A2 (0.88) → in-vocab (0.85) → OOV (0.50). Adds `cefr_level: "A2"` to lesson_data. No plugin code change needed — just populate the new dict in `cefr_vocab.py` and extend the check in each plugin.
- **CEFR B1 vocabulary** — **planned**: ~2000 lemmas per language. B1 ("threshold") covers abstract concepts, opinions, narrative structures. Expected to cover the bulk of vocabulary items that currently trigger OOV in real reading texts. Confidence 0.86. Same extension pattern.
- **CEFR B2–C2 vocabulary** — **planned**: B2 (~3500 lemmas), C1 (~5000), C2 (~8000+). At these levels domain-specific and literary vocabulary dominates; curation requires per-domain wordlists (academic, business, literary) in addition to general frequency lists. Consider integrating an open frequency corpus (OpenSubtitles, OSCAR) for automated candidate generation with human review.
...
- **Classical lexicon depth** — **partially done** (2026-05-27): Latin and Koine Greek now have offline morphological annotations (~3 400 Latin forms from UD ITTB; ~27 000 Greek forms from UD PROIEL + MorphGNT). Perseus Digital Library integration or Logeion API would further increase coverage of attested forms outside these corpora. Remains **planned** for higher-quality coverage.

...
| English plugin (full grammar/nuance) | implemented | `en_core_web_sm`; vocabulary, grammar (6 patterns: phrasal_verb, be_progressive, be_passive, have_perfect, cleft, existential), nuance (register markers); shallow tense morphology (present/past) |
...
- ~~**Replace `create_all` with `alembic upgrade head`**~~ — **done**: production startup runs `alembic upgrade head` only; `create_all` removed from all startup paths. — ~~**Private alpha blocker A1**~~
- ~~**`max_chars` guard on `/parse` and `/ingest`**~~ — **done**: `max_parse_chars` (default 10 000) in `Settings`; both routes raise 413 with a descriptive detail before any NLP runs; `/parse/jobs` uses the higher `max_job_chars` (500 000) ceiling. — ~~**Private alpha blocker A2**~~
- ~~**Data export endpoint**~~ — **done**: `GET /users/me/export` returns all `user_knowledge` rows (enriched with `canonical_form`/`type`/`display_label`) plus all language preferences as a versioned JSON document (`schema_version: "1"`). Covered by tests in `test_user_isolation.py`. — ~~**Private alpha blocker A4**~~
- ~~**JWT authentication**~~ — **done**: `POST /auth/register` + `POST /auth/login`; HS256 JWT; `get_current_user` verifies Bearer token or falls back to `X-User-Id`; per-user isolation across all routes. — ~~**Public beta blocker B1**~~
- ~~**Login/register UI**~~ — **done**: frontend login panel, JWT in `sessionStorage`, roving-tabindex auth tabs, logout button. — ~~**Public beta blocker B2**~~
...
- **User accounts** — **done**: JWT auth (`/auth/register` + `/auth/login`), login/register UI, JWT in `sessionStorage`, logout, per-user isolation across all routes, `UserLanguagePreferenceRow` preference CRUD.
...
- **PWA / offline mode** — **done**: `frontend/sw.js` caches app shell (cache-first for CSS/JS/HTML); `frontend/manifest.json` enables install; `frontend/js/offline.js` queues failed reviews in IndexedDB; drains queue on `window.online`. FastAPI now serves the frontend at `/` so the SW gets same-origin registration. **Offline status indicator** — **done**: `#offline-queue-badge` button in top-nav shows queue count with five states (offline/pending/syncing/synced/expired); JWT-expiry detection; `#offline-explain-dialog` with per-state explanations; forced-colors and reduced-motion support; i18n across all 11 locales. 33 tests in `frontend/tests/offline-badge.test.mjs`.
...
- **Shallow morphological plugins for Hindi and Turkish; rich Finnish morphology** — **done** (2026-05-27 onward): Hindi and Turkish remain suffix-rule `morphology_light` plugins. Hindi: Devanagari word regex, IAST romanisation, verb aspect/tense and noun case suffix rules, postposition tagging. Turkish: vowel-harmony-aware suffix rules for plural, case, tense, and mood (false-positive-prone short suffixes removed; confirmed tests pass). All three are covered by the harvest pipeline via FrequencyWords/OpenSubtitles. **Finnish upgraded (2026-05-29, deepened 2026-06-04):** stanza Finnish UD is primary with `fi_core_news_sm` fallback; `analysis_depth="full"`, `morphology_depth="rich"`, `morphology_quality="medium"`; 15 cases plus possessive suffixes, tense/mood/person/number/voice, polarity, vowel harmony, and consonant-gradation notes flow into `lesson_data`. Finnish now emits learner-facing grammar nuance/drill candidates for local cases, possessive suffixes, consonant gradation, passive voice, negative auxiliaries, and conditional mood. No spaCy model is available for Turkish or Hindi; those remain suffix-rule.
...
- **CEFR-graded lesson ordering** — **done** (2026-05-29): `GET /recommend` already returns `cefr_level` per sentence and accepts `?cefr=` filter. Frontend updated: CEFR level `<select>` (A1–C2 + All levels) in the parse dialog; persisted in `localStorage('mnemosyne.cefr.level')`; recommendation URL includes `?cefr=` when set; CEFR badge (`.rec-card__cefr-badge`) displayed on featured and alternative recommendation cards. Changing the CEFR select clears and reloads recommendations immediately.
...
- **Frontend UI string localisation** — **implemented**: all hardcoded English strings in frontend components replaced with `t()`/`ti()` calls sourced from `frontend/js/i18n.js`. Covers `mnemosyne-modal`, `mnemosyne-pill`, `mnemosyne-text-panel`, `mnemosyne-top-nav`, and `main.js` (aria-announces, drill feedback, rating labels). Grammatical terminal labels (`"third"`, `"singular"`, `"present"`) produced by `backend/lesson/generators.py` are translated via a lookup table in `l10n.py` for all 12 UI locales.
- **Classical lexicon depth** — **done** (2026-05-29): Latin noun/adj: ~257 000 forms (`la_morph.json`, kaikki); Latin verbs: ~615 000 forms in `la_verb_morph.db` (SQLite, 5 400 lemmas); Greek noun/adj: ~179 000 forms (`grc_morph.json`, kaikki); Greek verbs: ~415 000 forms in `grc_verb_morph.db` (SQLite, 1 750 lemmas; section-based tense). Latin `morphology_quality` `"low"` → `"medium"`. Greek `morphology_quality` remains `"medium"`. **Logeion done (2026-07-04):** `fetch_structured()` extracts `ls_definition`, `classical_citations`, `part_of_speech`, `gender` from L&S/LSJ; SQLite cache pre-fetched (8 807 entries: la 5 969, grc 2 838); la POS coverage 92.5%, grc 94.5%. **Perseus linked (2026-07-09):** `backend/dictionary/perseus.py` resolves L&S/LSJ citations to Scaife reader URLs for 22 mapped author abbreviations; lexicon badge in the detail pane links to Perseus morphological analysis for la/grc vocabulary items.

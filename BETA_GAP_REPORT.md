...
4. ~~**Analytics instrumentation call sites**~~ — **already done** (prior session): `maybe_record_event` wired in `parse.py` (`text_ingested`), `ingest.py` (`text_ingested`), `review.py` (`review_session`), `recommend.py` (`recommend_served`). All 4 valid event types covered.
